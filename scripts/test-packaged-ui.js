 ...
 *
 * 비개발자를 위한 설명:
 * - 개발 중에는 잘 되는데 설치 파일로 만들면 안 되는 경우가 종종 있습니다.
 *   (파일 경로가 달라지거나, 필요한 파일이 빠지는 등)
 * - 그래서 실제로 빌드된 exe 파일을 직접 실행해 화면이 정상인지 확인합니다.
 * - 여기서도 메뉴 이동만 하고, 로그인·생성·발행 버튼은 누르지 않습니다.
 *
 * 실행 전에 먼저 설치 파일을 빌드해 두어야 합니다.
 */
